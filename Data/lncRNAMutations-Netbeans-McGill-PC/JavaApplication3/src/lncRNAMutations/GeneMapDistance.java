...
//        compareGroups();
    ...
    public static void mapExpression() {
        String lncRNAtoGene[][]=DataSetReader.readDataSet("C:\\Users\\Dilmi\\Desktop\\Lowy\\Non-codingRNA\\Breast\\Breast-lncRNADB-GENCODE-to-fill.txt", 215, "\t");
        String geneExp[][]=DataSetReader.readDataSet("C:\\Users\\Dilmi\\Desktop\\Lowy\\Non-codingRNA\\Breast\\TMMnorm.txt",1216 , "\t");
        for (int i = 1; i < geneExp.length; i++) {
            for (int j = 1; j < lncRNAtoGene.length; j++) {
                if(geneExp[i][0].equals(lncRNAtoGene[j][1])){
                    for (int k = 1; k < geneExp[0].length; k++) {
                        for (int l = 2; l < lncRNAtoGene[0].length; l++) {
        ...
        DataSetWriter.writeToFile2D("C:\\Users\\Dilmi\\Desktop\\Lowy\\Non-codingRNA\\Breast\\Breast-lncRNADB-GENCODE-to-filled.txt", lncRNAtoGene);
       
    }
    
    public static void mapMutations() {
        String lncRNAtoSample[][]=DataSetReader.readDataSet("C:\\Users\\Dilmi\\Desktop\\Lowy\\Non-codingRNA\\Melanoma\\Melanoma-mutations-sample-to-lncRNA.txt", 11, "\t");
        String mutationMap[][]=DataSetReader.readDataSet("C:\\Users\\Dilmi\\Desktop\\Lowy\\Non-codingRNA\\Melanoma\\Melanoma-lncRNADB-to-mutations.txt", 37, "\t");
        for (int i = 0; i < lncRNAtoSample.length; i++) {
            for (int j = 1; j < mutationMap.length; j++) {
                if(lncRNAtoSample[i][3].equals(mutationMap[j][0])){
        ...
        DataSetWriter.writeToFile2D("C:\\Users\\Dilmi\\Desktop\\Lowy\\Non-codingRNA\\Melanoma\\Melanoma-to-lncRNA-mutaitonMap.txt", mutationMap);
    }
    
    public static void compareGroups() {
        String lncRNAtoGene[][]=DataSetReader.readDataSet("C:\\Users\\Dilmi\\Desktop\\Lowy\\Non-codingRNA\\Lung\\Lung-lncRNADB-GENCODE-to-filled.txt", 108, "\t");
        String mutationMap[][]=DataSetReader.readDataSet("C:\\Users\\Dilmi\\Desktop\\Lowy\\Non-codingRNA\\Lung\\Lung-to-lncRNA-mutaitonMap.txt", 107, "\t");
        String output[][]=DataSetReader.readDataSet("C:\\Users\\Dilmi\\Desktop\\Lowy\\Non-codingRNA\\Lung\\lncRNADB-GENCODE-map-p-value.txt", 6, "\t");
        ...
        for (int i = 1; i < output.length; i++) {
            ...
            for (int j = 1; j < lncRNAtoGene.length; j++) {
                if(lncRNA.equals(lncRNAtoGene[j][0]) && gene.equals(lncRNAtoGene[j][1]) ){
                    //System.out.print(""+lncRNA);
                    //System.out.println("\t"+gene);
                    for (int k = 2; k < lncRNAtoGene[0].length; k++) {
                        geneExpressionMatrix[k-2]=lncRNAtoGene[j][k];
                    }
                    break;
                }
            }
        ...
        DataSetWriter.writeToFile2D("C:\\Users\\Dilmi\\Desktop\\Lowy\\Non-codingRNA\\Lung\\Lung-lncRNADB-GENCODE-map-p-value-foldChange-filled.txt", output);
    }
