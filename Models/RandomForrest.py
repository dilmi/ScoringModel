...
#tune_params = [{'n_estimators': [10, 20, 50, 100, 150, 200, 250, 300, 400, 500]}]
tune_params = [{'n_estimators': [10, 200, 300,  500]}]
final_Results_mat = []
fold = 0
for train_index, test_index in kf:
    X_train_kf, X_test_kf, y_train_kf, y_test_kf = X_train[train_index], X_train[test_index], y_train[train_index], \
                                                   y_train[test_index]
    gsCV = GridSearchCV(RandomForestClassifier(n_estimators=100, class_weight='auto'), tune_params,
                        cv=len(tune_params[0]['n_estimators']), n_jobs=-1)
